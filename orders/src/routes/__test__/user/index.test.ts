...
it("fetches orders for a particular user", async () => {
  ...
  const productOne = await buildProduct("prod_1");
  const productTwo = await buildProduct("prod_2");
  const productThree = await buildProduct("prod_3");
...
it("200 for authenticated user fetches correct orders", async () => {
  // Setup
  const productA = await buildProduct("prod_a");
  const productB = await buildProduct("prod_b");
